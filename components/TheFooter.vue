    ...
    <div class="max-w-7xl border-l border-r border-dashed border-gray-700 mx-auto px-4">
      <div class="pt-5 pb-12 md:flex md:items-center md:justify-between">
        <div class="flex space-x-6 md:order-2">
          <a v-if="$store.state.social.facebook" target="_blank" rel="noreferrer"
            :href="`https://facebook.com/${$store.state.social.facebook}`" class="text-gray-400 hover:text-gray-300">
          ...
          <a v-if="$store.state.social.instagram" target="_blank" rel="noreferrer"
            :href="`https://instagram.com/${$store.state.social.instagram}`" class="text-gray-400 hover:text-gray-300">
          ...
          <a v-if="$store.state.social.twitter" target="_blank" rel="noreferrer"
            :href="`https://twitter.com/${$store.state.social.twitter}`" class="text-gray-400 hover:text-gray-300">
            <span class="sr-only">Twitter</span>
            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" version="1.1"
              xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" viewBox="0 0 16 16">
              <g transform="matrix(0.6666666666666666,0,0,0.6666666666666666,0,0)">
                <path
                  ...
                  stroke="none" fill="currentColor" stroke-width="0" stroke-linecap="round" stroke-linejoin="round">
                </path>
              </g>
            </svg>
          </a>
          <a v-if="$store.state.social.github" target="_blank" rel="noreferrer"
            :href="`https://github.com/${$store.state.social.github}`" class="text-gray-400 hover:text-gray-300">
          ...
          <a v-if="$store.state.social.linkedin" target="_blank" rel="noreferrer"
            :href="`https://linkedin.com/in/${$store.state.social.linkedin}`" class="text-gray-400 hover:text-gray-300">
        ...
        <p class="mt-8 text-sm leading-6 text-gray-400 md:mt-0 md:order-1">
          {{ `${$store.state.name} © 2017 - ${new Date().getFullYear()}` }} • <nuxt-link
            :to="localePath('terms')">terms</nuxt-link> • <nuxt-link :to="localePath('privacy')">privacy</nuxt-link> •
          template by <span class="ceator-preview hover:text-hot-pink text-gray-300">
            <a class="hidden md:inline-block" rel="noreferrer" target="_blank"
              href="https://github.com/w3security/w3security.github.io">
              <creator-preview :avatar="avatar" :name="name" :description="description" :content="content" :mode="mode" />
            </a>
            <a class="inline-block md:hidden" target="_blank" href="https://github.com/w3security/w3security.github.io">{{
              name }}
            </a>
          </span>
        </p>
...
export default {
  ...
  data() {
    return {
      avatar: 'https://cdn.w3security.github.io/w3security.jpg',
      name: 'W3security',
      mode: 'dark',
      description: 'Software Engineer who tinkers with side projects, every now n then.',
      content: `Hey, I am W3security 👋<br/> I built this template for procrastinating devs to set up their portfolio + blogs<br/><br/>
