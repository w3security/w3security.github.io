          ...
          <div class="space-y-9">
            <div class="py-0.5 border-t border-b border-dashed border-gray-700 flex items-center justify-between">
              <div class="text-indigo-700 font-bold">
                <nuxt-link class="hover:text-indigo-500" :to="localePath('/projects')">{{ $t('projects.header')
                                  }}</nuxt-link> <span class="text-gray-700">/</span> {{ project.tech }}
              </div>
              <div><a target="_blank" rel="noreferrer" :href="project.twitterShareUrl"
                ...
                <div class="space-y-1 text-center">
                  ...
                  <dl class="">
                    <div>
                      ...
                      <dd class="text-xs font-medium text-gray-500">
                        <a target="_blank" rel="noreferrer" :href="project.website"><span
                            class="text-indigo-600 hover:text-indigo-300">{{ project.title }}</span></a>
                        <span class="mx-1">
                          &middot;
                        </span>
                        {{ project.readingTime }}
                  ...
                  <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div
                      ...
                      v-for="(tag, index) in project.tags" :key="`tag-${index}`">{{ tag }}</div>
                  </div>
                </div>
              </header>
...
export default {
  ...
  head() {
    return {
      title: this.project.title + ' | projects -- W3security | Friendly Neighborhood Developer',
      meta: [
