          ...
          <div class="space-y-9">
            <div class="py-0.5 border-t border-b border-dashed border-gray-700 flex items-center justify-between">
              <div class="text-indigo-700 font-bold">
                <nuxt-link class="hover:text-indigo-500" :to="localePath('/blog')">{{ $t('blog.header') }}</nuxt-link>
                <span class="text-gray-700">/</span> {{ article.category }}
              </div>
              <div><a target="_blank" rel="noreferrer" :href="article.twitterShareUrl"
                  class="inline-flex items-center py-2 px-3 space-x-2 rounded-md bg-gray-900 text-gray-300 text-xs font-medium hover:text-gray-200 transition ease-in-out duration-100"><span>Share
                    on Twitter</span><svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
            ...
            <div class="px-4 sm:px-0 lg:px-4 pb-4 bg-gray-900 rounded-lg">
              <a href="https://twitter.com/gyanw3security">
                <div class="flex flex-items-center justify-center  rounded-full">
                  <img class="w-12 h-12 border-2 border-indigo-600 hover:border-hot-pink -mt-5 bg-gray-900 rounded-full"
                    :src="article.author.image" alt="Author">
                </div>
              </a>
              <header class="py-4">
                <div class="space-y-1 text-center">
                  ...
                  <dl class="">
                    <div>
                      ...
                      <dd class="text-xs font-medium text-gray-500">
                        <a target="_blank" rel="noreferrer" :href="`https://twitter.com/${article.author.twitter}`"><span
                            class="text-indigo-600 hover:text-indigo-300">{{ article.author.name }}</span></a>
                        <time :datetime="article.createdAt">{{ ' on ' + new
                  ...
                  <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div
                      ...
                      v-for="(tag, index) in article.tags" :key="`tag-${index}`">{{ tag }}</div>
                  </div>
                </div>
              </header>
...
export default {
  ...
  head() {
    return {
      title: this.article.title + ' | Blog -- W3security | Friendly Neighborhood Developer',
      meta: [
